...
| **Standalone courses** | Long-form, module-by-module courses that go noob → pro: **JavaScript**, **CSS & HTML**, **TypeScript**, **React**, **DSA** (14 modules / 86 lessons), **Node.js** (10 modules / 68 lessons), **Python** (10 modules / 60 lessons), **Django + DRF** (13 modules / 78 lessons), and **Databases** (20 modules / 120 lessons — COMPLETE: SQL core (1-10) + PostgreSQL (11-12) + MongoDB (13-16) + Firebase — Firestore & Realtime Database (17-18) + Redis & SQLite (19) + the database-landscape survey — columnar, graph, time-series, search, wide-column, CAP/PACELC, choosing from access patterns, migrations & operations (20)), and **DevOps** (20 modules planned, advanced — delivery lifecycle & DORA, Linux & networking, containers & Kubernetes, CI/CD, IaC, cloud, observability, reliability, DevSecOps, GitOps/platform/FinOps; Modules 1-19 live — verified examples run against a real tool: shell/openssl (Modules 2-3), git (Module 4), Docker build+run (Module 5), Docker Compose (Module 6), a real Kubernetes cluster + ingress-nginx + metrics-server + Helm (Modules 7-9), GitHub Actions run for real with `act` + linted with `actionlint` (Module 10), Argo Rollouts canary pause/promote/abort + blue-green Service cutover + real PostgreSQL expand/contract migrations (Module 11), real `terraform` apply/plan/state with the local + random providers (Module 12), the real Prometheus toolchain — `promtool check config` / `check rules` / `test rules` (unit-testing PromQL and alert rules with synthetic series and exact assertions), `amtool` routing tests, and `otelcol validate` (Module 16), availability / retry-backoff / token-bucket sims plus `k6 inspect` load-script validation (Module 17), the real supply-chain toolchain — `syft` SBOM generation, `trivy` SCA / secret / Dockerfile-misconfig scanning, and `cosign` sign/verify with tamper detection (Module 18), and a real HashiCorp `vault` (dev-mode ACL policies, scoped short-lived tokens, the transit engine) plus `sops` + `age` encrypting a Kubernetes Secret manifest (Module 19); Modules 13-15 (Cloud Fundamentals, Cloud in Practice, Observability) are prose + realistic output, AWS worked with concrete Azure equivalents. Every lesson follows the same shape — a real-life analogy, a broken example, the fix, a deep dive, worked examples with runnable output, common mistakes, real-world use, interview Q&A, exercises, and key takeaways — in English and Hinglish. Data lives in `server/prisma/seed-data/course-*.ts`; every code sample is executed and diffed against its stated output before it ships |
